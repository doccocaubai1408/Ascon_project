...
void check_and_print_total() {
    ...
    if (should_print) {
        ...
        Serial.printf("Core 0 - Encryption: %.2f MB/s, Decryption: %.2f MB/s, Hash: %.2f MB/s\n",
                      core_stats[0].enc_throughput, core_stats[0].dec_throughput, core_stats[0].hash_throughput);
        Serial.printf("Core 1 - Encryption: %.2f MB/s, Decryption: %.2f MB/s, Hash: %.2f MB/s\n",
                      core_stats[1].enc_throughput, core_stats[1].dec_throughput, core_stats[1].hash_throughput);
...
void benchmark_ascon() {
    ...
    unsigned char ad[16] = {0};  // Associated data    
...
void IRAM_ATTR benchmark_core(void* parameter) {
  ...
  int ramBefore = freeRam();
    ...
    if (!message || !ciphertext || !decrypted || !hash || !message1 || !ciphertext1) {
        ...
        if (message1) heap_caps_free(message1);
        if (ciphertext1) heap_caps_free(ciphertext1);
        vTaskDelete(NULL);
        return;
    }
   ...
   crypto_hash(hash, message1, MESSAGE_SIZE1);
